...
const MAX_VARIANTS_PER_REF: usize = 8;
const PREFIX_VARIANT_TRY_LIMIT: usize = 3;
const FUZZY_VARIANT_TRY_LIMIT: usize = 2;
...
fn prune_variants(order: Vec<String>) -> Vec<String> {
    if order.len() <= MAX_VARIANTS_PER_REF {
        return order;
    }
    let mut pruned = Vec::with_capacity(MAX_VARIANTS_PER_REF);
    let mut seen = HashSet::new();
    for variant in order {
        if seen.insert(variant.clone()) {
            pruned.push(variant);
            if pruned.len() >= MAX_VARIANTS_PER_REF {
                break;
            }
        }
    }
...
struct RefCtx {
    ...
    variants: Vec<String>,
    /// כמה פעמים ההפניה הזו הופיעה בטקסט; נבדקת פעם אחת, ותוצג כפול
...
fn get_references_with_context(text: &str, brackets: &str) -> Vec<RefCtx> {
    ...
    for cap in re.captures_iter(text) {
        ...
        let count = seen.entry(reference.clone()).or_insert(0);
        *count += 1;
        ...
        } else if let Some(existing) = refs.iter_mut().find(|r| r.reference == reference) {
...
async fn call_gemini(
    ...
    if let Some(finish) = candidate.and_then(|c| c.get("finishReason")).and_then(|r| r.as_str()) {
        ...
        if finish == "MAX_TOKENS" {
            // בעבר טופל כ"בסדר, נחזיר מה שיש" - וזה גרם לתשובות חתוכות
            // באמצע משפט/מקור להיראות כהצלחה מלאה, בלי שום סימון למשתמש.
            // עכשיו מטופל ככישלון מפורש (כמו SAFETY), כך שה-JS יסמן את
            // הקטע כ"נכשל" וישמור את המקור, במקום לספק תוצאה חתוכה
            // כאילו היא שלמה.
            return Err(
                "התשובה הגיעה למגבלת האורך המקסימלי (8192 טוקנים) ונחתכה באמצע.\nנסה קטע קצר יותר.".to_string()
            );
        }
    }
...
fn scan_chunk(
    ...
    for (i, rc) in chunk.iter().enumerate() {
        ...
        if !has_exact {
            for v in variants.iter().take(PREFIX_VARIANT_TRY_LIMIT) {
            ...
            if !has_prefix && !out.is_empty() {
                for v in variants.iter().take(FUZZY_VARIANT_TRY_LIMIT) {
                    if v.chars().count() < 4 {
                        continue;
                    }
                    let rows_before = out.len();
...
mod tests {
    ...
    #[test]
    fn generate_variants_stays_small_for_fast_lookup() {
        let variants = generate_variants("ברכות ב, ע\"א");
        assert!(variants.len() <= MAX_VARIANTS_PER_REF);
        assert!(variants.iter().any(|v| v.contains("ברכות")));
    }
